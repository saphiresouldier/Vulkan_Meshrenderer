...
void ViewerApplication::run()
{
  ...
  initVulkan(windowingSystem);
  mainLoop();
  cleanup();
}

void ViewerApplication::initVulkan(Saphire::WindowingSystem winSys) {
  createInstance();
...
void ViewerApplication::mainLoop() {
  while (!windowingSystem.shouldWindowClose()) {
    ...
    drawFrame(windowingSystem);
  }

  vkDeviceWaitIdle(device);
}

void ViewerApplication::drawFrame(Saphire::WindowingSystem winSys) {
...
void ViewerApplication::recreateSwapChain(Saphire::WindowingSystem winSys) {
...
void ViewerApplication::createSurface(Saphire::WindowingSystem winSys) {
  if (winSys.createVulkanWindowSurface(instance, nullptr, &surface) != VK_SUCCESS) {
    throw std::runtime_error("failed to create window surface");
  }
}

void ViewerApplication::pickPhysicalDevice() {
...
VkExtent2D ViewerApplication::chooseSwapExtent(Saphire::WindowingSystem winSys, const VkSurfaceCapabilitiesKHR& capabilities) {
...
void ViewerApplication::createSwapChain(Saphire::WindowingSystem winSys) {
...
std::vector<char> ViewerApplication::readFile(const std::string& filename) {
...
uint32_t ViewerApplication::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
